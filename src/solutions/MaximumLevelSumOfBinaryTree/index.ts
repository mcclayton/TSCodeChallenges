...
function maximumLevelSumOfBinaryTree(root: TreeNode | null): number {
  ...
  const res = Object.entries(levelSumMap).reduce(
    (maxSumMap: Record<string, any> | null, [levelKey, sum]) => {
...
function traverse(
  node: TreeNode | null,
  level: number,
  levelSumMap: Record<number, number>,
  ...
  levelSumMap[level] = levelSumMap[level]
